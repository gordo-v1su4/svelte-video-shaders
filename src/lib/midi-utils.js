...
function parseMIDIBuffer(dataView) {
	...
	for (let trackIndex = 0; trackIndex < numTracks; trackIndex++) {
		...
		while (offset < trackEnd) {
			...
			if (statusByte < 0x80) {
				...
				console.warn(
					`[MIDI] Running status detected at offset ${offset}, skipping (not fully supported)`
				);
			...
			if (statusByte === 0xff) {
				...
				if (metaType === 0x51 && metaLength === 3) {
					...
					currentTimeSeconds = (currentTime / ticksPerQuarterNote) * (tempo / 1000000);
					console.log(
						`[MIDI] Tempo change at ${currentTimeSeconds.toFixed(3)}s: ${oldTempo} -> ${tempo} (${60000000 / tempo} BPM)`
					);
				} else {
	...
	if (result.times.length > 0) {
		console.log(
			`[MIDI] Time range: ${result.times[0].toFixed(3)}s - ${result.times[result.times.length - 1].toFixed(3)}s`
		);
		console.log(
			`[MIDI] Sample times:`,
			result.times.slice(0, 5).map((t) => t.toFixed(3))
		);
	}
